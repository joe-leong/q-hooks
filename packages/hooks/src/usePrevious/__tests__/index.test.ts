...
import type { ShouldUpdateFn } from '..';
import usePrevious from '..';

describe('usePrevious', () => {
  function getHook<T>(initialValue?: T, compareFunction?: ShouldUpdateFn<T>) {
    return renderHook(({ val, cmf }) => usePrevious<T>(val as T, cmf), {
      initialProps: {
        val: initialValue || 0,
        cmf: compareFunction,
      } as { val?: T; cmf?: ShouldUpdateFn<T> },
    });
  }
  ...
  it('should receive a predicate as a second parameter that will compare prev and current', () => {
    ...
    hook.rerender({ val: obj2, cmf: predicate });

    expect(hook.result.current).toBeUndefined();

    hook.rerender({ val: obj3, cmf: predicate });
